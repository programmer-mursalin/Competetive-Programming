...
void solve()
{
    ...
    if (v2.size() > 0)
    {
        ...
        {
            auto lb = lower_bound(ALL(v1), v2[i]);
            if (v1.empty())
                continue;

            if (lb == v1.end())
            {
                maxi = max(maxi, mp2[v2[i]] + mp1[v1.back()]);
            }
            else
            {
                int index = lb - v1.begin();
                if (index - 1 >= 0)
                    maxi = max(maxi, mp2[v2[i]] + mp1[v1[index - 1]]);
                else
                    maxi = max(maxi, mp2[v2[i]]);
            }
        }
    }
    ...
    for (int i = 0; i < (int)v1.size(); i++)
    {
        if (v2.empty())
            break;
        auto ub = upper_bound(ALL(v2), v1[i]);
        if (ub == v2.end())
            continue;

        int index = ub - v2.begin();
        if (index >= 0 && index < (int)v2.size())
            maxi = max(maxi, mp1[v1[i]] + mp2[v2[index]]);
    }

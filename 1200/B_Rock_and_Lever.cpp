...
void solve()
{
    ...
    // Map to track the count of occurrences of first set bit positions
    map<int, int> mp;
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        // Use bitset to find the first set bit position in the number a[i]
        int x = a[i];
        for (int j = 31; j >= 0; j--) // Loop from the highest bit to 0
        {
            if ((x & (1 << j)) != 0)
            {
                position = j;
        ...
        // Increase the count of the first set bit position
        mp[position]++;
    }

    // Now calculate the number of pairs
    int count_pairs = 0;
    for (auto it : mp)
    {
        ...
        if (occurrences > 1)
        {
            // For each bit position, the number of ways to form pairs is C(n, 2) = n * (n - 1) / 2
            count_pairs += (occurrences * (occurrences - 1)) / 2;
        }
    }

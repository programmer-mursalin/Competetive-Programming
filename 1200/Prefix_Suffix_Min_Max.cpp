...
void solve()
{
    ...
    int maxi = 3; // Ensure this constant has a clear meaning or make it dynamic.
    int st = 0;   // Represents the current state of sum or transformation.

    vector<int> ans(n);
    for (int i = 0; i < n; i++)
    {
        int x1 = a[i] + N - st; // Transformation step
        cout << x1 << " ";
        st = st + x1; // Update the state by adding the newly calculated x1
    }

...
void solve()
{
    ...
    for (int i = 0; i < n - 1; i++)
    {
        int lower_bound_value = sum - l - a[i];

        int upper_bound_value = sum - h - a[i];
        if (upper_bound_value < 0)
            upper_bound_value = a[i + 1];

        auto it1 = upper_bound(a.begin() + i + 1, a.end(), lower_bound_value) - a.begin();
        auto it2 = lower_bound(a.begin() + i + 1, a.end(), upper_bound_value) - a.begin();
        // if (upper_bound_value < 0)
        // {
        //     ans += it1;
        //     continue;

...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (ms.count(flipped))
        {
            // If flipped value is found, it means we can pair it
            ms.erase(ms.find(flipped)); // Erase the one occurrence of flipped value
            ans--;                      // Decrease answer for the successful pair
        }
        else
        {
            // Otherwise, insert the current number
            ms.insert(x);
        }
    }

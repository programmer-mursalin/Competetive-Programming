...
void solve()
{
    ...
    vector<pair<int, int>> vp(n); // Corrected declaration of vector of pairs
    for (int i = 0; i < n; i++)
    {
        vp[i] = {b[i], i}; // Filling the vector of pairs correctly
    }
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (vp[i].first < m)
        {
            ...
            if (prn - a[vp[i].second] > 0)
            {
                cost += a[vp[i].second] * vp[i].first;
                prn -= a[vp[i].second];
            }
            else
            {

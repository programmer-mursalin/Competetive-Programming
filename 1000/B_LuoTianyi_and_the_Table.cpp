...
void solve()
{
    ...
    sort(ALL(a));  // Sorting the entire array
    int x = n * m; // Total number of elements
    int c = 1;
    if (a[x - 1] - a[1] > a[x - 2] - a[0])
        c = a[x - 1] - a[1];
    else
        c = (a[x - 2] - a[0]);

    if (a[0] == a[1]) // If the smallest two elements are the same
    {
        cout << (a[x - 1] - a[0]) * (x - 1) << endl; // Use x instead of n for full array
    }

    else
    {
        cout << (c * (min(n, m) - 1)) + ((a[x - 1] - a[0]) * ((x - 1) - (min(n, m) - 1))) << endl;
    }
}
